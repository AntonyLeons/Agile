...
try  
        {  
          ...
          $serverName = "sql.rde.hull.ac.uk";  
            ...
            $FirstName="SELECT FirstName FROM ACW WHERE StudentID='$StudentID'";
            $Surname="SELECT Surname FROM ACW WHERE StudentID='$StudentID'";
            $UserType="SELECT UserType FROM ACW WHERE StudentID='$StudentID'";

            $FirstName= sqlsrv_query($conn, $FirstName);
            $Surname= sqlsrv_query($conn, $Surname);
            $UserType= sqlsrv_query($conn, $UserType);
...
try  
        {   $serverName = "sql.rde.hull.ac.uk";  
            ...
            if (empty($_POST["StudentID2"])) {
                ...
                $StudentID2 = test_input($_GET["StudentID2"]);
                // check if name only contains letters and whitespace
                if (!preg_match("/^[1-9][0-9]*$/",$StudentID2)) {
                  $StudentIDErr = "Only Numbers allowed in StudentID"; 
                }
              }
            $Fetch="SELECT TOP 1 [Location] FROM ACW Where [StudentID]=? ORDER BY [ID] DESC";
            $params = array($StudentID2);
