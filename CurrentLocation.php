...
<div class="message">
        ...
        <ul>
			...
			<li><a class="active" href="CurrentLocation.html">Current Location</a></li>
...
<?php
  function alert($msg)
  {
    echo "<script type='text/javascript'>alert('$msg');location='CurrentLocation.html';</script>";
  }
         ...
         $StudentID = $FirstName = $Surname = $Location = $UserType = "";
          $serverName = "sql.rde.hull.ac.uk";  
            ...
            if(($StudentIDErr =="") && ($FirstNameErr =="") && ($SurnameErr =="") && ($LocationErr =="") && ($UserTypeErr == ""))
              {
                $Fetch="SELECT * FROM ACW WHERE (?='' OR StudentID LIKE '%' + ? + '%') AND (?='' OR FirstName LIKE '%' + ? + '%') AND (?='' OR Surname LIKE '%' + ? + '%') AND (?='' OR Location LIKE '%' + ? + '%') AND (?='' OR UserType LIKE '%' + ? + '%') AND (ID IN (SELECT MAX(ID) FROM [ACW] GROUP BY StudentID)) Order by ID DESC";
              }
              else
              {
